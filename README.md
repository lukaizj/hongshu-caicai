...
| Cookie 管理 | 页面内查看 Cookie 状态、手机验证码获取、手动粘贴更新、清除当前 Cookie |
...
也可以启动采集台后，在页面「一键获取 Cookie」区域输入手机号，发送并填写短信验证码，验证成功后自动保存 Cookie。

如果验证码登录不可用，可在「手动替换 Cookie」输入框中粘贴并保存。手动 Cookie 获取方式：浏览器登录小红书网页版后，打开开发者工具，进入 Network / Fetch/XHR，选择任意接口请求，复制请求头里的 `cookie` 字段。
...
3. 在 Cookie 状态区确认 Cookie 可用；失效时使用手机验证码登录自动更新 Cookie。
4. 如果验证码登录不可用，粘贴浏览器 Cookie 并保存。
5. 输入关键词，例如 `苏州探店`。
6. 设置采集篇数，当前单次限制 1-50 篇。
7. 如需评论，勾选「同步采集评论」。首次建议 1-10 篇小批量测试。
8. 点击「开始采集」，等待实时进度完成。
9. 下载生成的笔记 Excel 和评论 Excel。
